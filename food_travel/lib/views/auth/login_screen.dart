...
class _LoginScreenState extends State<LoginScreen> {
    ...
    try {
      ...
      unawaited(
        _ensureUserProfile(
          user,
          email,
        ).timeout(const Duration(seconds: 12)).catchError((e, _) {
      ...
      if (!mounted) return;
      setState(() => _formError = _mapLoginError(t, e));
    } catch (e) {
      if (!mounted) return;
      setState(
        () =>
  ...
  Future<void> _handleGoogleLogin() async {
    ...
    try {
      final cred = await _authService.signInWithGoogle().timeout(
      ...
      setState(
        () =>
            _formError =
                _isVi
                    ? 'Đăng nhập Google quá lâu. Vui lòng kiểm tra mạng.'
                    : 'Google sign-in timed out. Please check your network.',
      );
    } on FirebaseAuthException catch (e) {
      if (e.code == 'sign_in_canceled') return;
      if (!mounted) return;
      final t = AppLocalizations.of(context)!;
      setState(() => _formError = _mapGoogleError(t, e));
    } catch (e) {
      if (!mounted) return;
      setState(
        () =>
